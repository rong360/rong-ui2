...
function telephoneClearNonNumbers (str) {
  // 先把非数字的都替换掉，除了数字和.
  str = str.replace(/[^\d]/g, "")
  // 国内手机号1开头，第二位是3,4,5,7,8,9, 并且11位
  if ((str.length == 1 && str != 1) || (str.length >= 2 && !/^[1][3,4,5,7,8,9]/g.test(str))) {
    str = "1"
  } else {
    str = str.substr(0, 11)
  }
...
export default {
  ...
  computed: {
    ...
    showClear () {
      return this.focused && this.currentValue.length > 0
    },
    errorCls () {
  ...
  methods: {
    ...
    onFieldBlur (e) {
      ...
      if (this.attrs.type == 'number') {
        if (this.attrs.fixed > 1 && currentValue != '') {
          // 保留n位小数，如：2，会在2后面补上00.即2.00
          let dotPos = currentValue.indexOf('.')
          if (dotPos < 0) {
            currentValue += '.'
            dotPos = currentValue.length - 1
          }
          while (currentValue.length <= dotPos + this.attrs.fixed) {
            currentValue += '0'
          }
        } else {
          // 不能以'.'结尾
    ...
    onFieldInput (e) {
      ...
      if (this.attrs.maxlength && e.target.value.length > this.attrs.maxlength) {
        currentValue = e.target.value.slice(0, this.attrs.maxlength)
      }
      if (this.attrs.type == 'number') {
        currentValue = clearNonNumbers(currentValue)

        // 限制小数位数
        let dotPos = currentValue.indexOf('.')
        if (this.attrs.fixed > 0 && dotPos > -1) {
          currentValue = currentValue.substring(0, dotPos + this.attrs.fixed + 1)
        } else if (this.attrs.fixed <= 0 && dotPos > -1) {
          currentValue = currentValue.substring(0, dotPos)
        }
        // 解决ios输入非数字时value清空问题
        if (currentValue == '' && this.prevValue != currentValue && !this.isBackSpace) {
          currentValue = this.prevValue
        }
      } else if (this.attrs.type == 'tel') {
        currentValue = telephoneClearNonNumbers(currentValue)
      } else if (this.attrs.type == 'IDCard') {
        currentValue = IDCardClearNonNumbers(currentValue)
      }
      this.currentValue = currentValue
    ...
    onFieldKeyup (e) {
      if (this.attrs.type == 'number' && !this.isAndroid && this.currentValue == '' && this.prevValue == '' && e.keyCode != 189) e.target.value = ''
